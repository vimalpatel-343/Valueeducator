        ...
        <div class="row">
            <div class="col-lg-6 col-12">
                ...
                <?php else: ?>
                    ...
                    <?php if(is_array($portfolioDistribution) && count($portfolioDistribution) > 0) { ?>
                    ...
                    <div class="white-box sc-mt-0 d-lg-flex justify-content-between align-items-center" style="padding:25px 0px 0px; margin-top:5px;">
                        <div class="container">
                            <div class="row">
                                ...
                                <div class="col-lg-6 col-6" style="padding-right:0px;">
                                    <h3 class="font-lg-14-normal sc-mb-0" style="color:#8D8D8D;">Last recommendation date</h3>
                                    <p class="font-lg-16-bold sc-mb-0"><?= date('M d, Y', strtotime($portfolioOverview['fld_last_recommendation_date'] ?? 'now')) ?></p>
                                </div>

                                <div class="col-lg-6 col-6" style="padding-right:0px;">
                                    <h3 class="font-lg-14-normal sc-mb-0" style="color:#8D8D8D;">Top 3 Sectors Represented</h3>
                                    <p class="font-lg-16-bold sc-mb-20"><?= $portfolioOverview['fld_top_sectors'] ?? '80' ?></p>
                                </div>

                                <div class="col-lg-6 col-6" style="padding-right:0px;">
                                    <h3 class="font-lg-14-normal sc-mb-0" style="color:#8D8D8D;">Upcoming Portfolio Review</h3>
                                    <p class="font-lg-16-bold sc-mb-0"><?= $portfolioOverview['fld_upcoming_review'] ?? '20' ?></p>
                                </div>

                                <div class="col-lg-6 col-6" style="padding-right:0px;">
                                    <h3 class="font-lg-14-normal sc-mb-0" style="color:#8D8D8D;">Average Market Cap</h3>
                                    <p class="font-lg-16-bold sc-mb-20"><?= $portfolioOverview['fld_average_market_cap'] ?? 'Quarterly' ?></p>
                                </div>

                                <div class="col-lg-6 col-6" style="padding-right:0px;">
                                    <h3 class="font-lg-14-normal sc-mb-0" style="color:#8D8D8D;">Dependency on US Economy</h3>
                                    <p class="font-lg-16-bold sc-mb-0"><?= $portfolioOverview['fld_dependency_on_us_economy'] ?? 'Mar 2026' ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="container outerbox" style="padding:20px;">
            <div class="sc-heading-area sc-mb-20 text-left">
                <div class="d-flex justify-content-between align-items-center sc-mb-0 sc-md-mb-0">
                    <h3 class="font-lg-20-bold font-20-bold sc-mb-0">Tiny Titans Portfolio Updates</h3>
                    <?php if ($hasAccess): ?>
                        <a href="<?= base_url('quarterly-updates-tiny-titan') ?>" class="more font-lg-16-bold">See All</a>
                    <?php else: ?>
...
<div class="sc-philosophy-area sc-md-pb-60 sc-md-mt-0 sc-mb-40">
    <div class="container">
                        ...
                        <table class="table">
                            <thead class="table__thead">
                                <tr>
                                    ...
                                    <th class="table__th" style="width: 8%;">Rating</th>
                                ...
                                <?php $i=1; foreach ($modelPortfolio as $stock): ?>
                                    <tr class="table-row">
                                        ...
                                        <td class="table-row__td" data-column="Progress">
                                            ...
                                            ?>
                                            <?php if ($hasUpdates): ?>
                                                <img src="<?= base_url('images/icon-bell.svg') ?>" width="24" class="stock-link" data-product-id="2" data-stock-id="<?= $stock['id'] ?>" data-stock-name="<?= htmlspecialchars($stock['fld_stock_name']) ?>" style="cursor: pointer;">
                                            <?php else: ?>
...
<script>
    ...
    // Stock update icon click handler
    document.addEventListener("DOMContentLoaded", function() {
        document.querySelectorAll('.stock-update-icon').forEach(icon => {
            icon.addEventListener('click', function() {
                const stockId = this.getAttribute('data-stock-id');
                const stockName = this.getAttribute('data-stock-name');
                
                // Fetch stock updates for this stock via AJAX
                fetch(`<?= base_url('api/stock-updates/') ?>${stockId}`)
                    .then(response => response.json())
                    .then(data => {
                        if (data.status && data.updates.length > 0) {
                            let content = '<div class="tabs_wrapper">';
                            content += '<ul class="tabs-company">';
                            
                            data.updates.forEach((update, index) => {
                                const activeClass = index === 0 ? 'active' : '';
                                const formattedDate = new Date(update.date).toLocaleDateString('en-IN');
                                content += `<li class="${activeClass}" rel="tab${index}">${formattedDate}</li>`;
                            });
                            ...
                            data.updates.forEach((update, index) => {
                                const activeClass = index === 0 ? 'd_active' : '';
                                const formattedDate = new Date(update.date).toLocaleDateString('en-IN');
                                content += `<h3 class="${activeClass} tab_drawer_heading" rel="tab${index}">${formattedDate}</h3>`;
                                content += `<div id="tab${index}" class="tab_content">`;
                                content += `<div class="font-lg-16">${update.description || 'No description available'}</div>`;
                                content += '</div>';
                            });
                            
                            content += '</div></div>';
                            
                            document.querySelector('.stock-modal-title').textContent = stockName;
                            document.querySelector('.tabs_wrapper').innerHTML = content;
                            ...
                            // Tab functionality
                            document.querySelectorAll('.tabs-company li').forEach(tab => {
                                tab.addEventListener('click', function() {
                                    ...
                                    document.querySelectorAll('.tabs-company li').forEach(t => {
                                        t.classList.remove('active');
                                    });
                                    this.classList.add('active');
                                    
                                    document.querySelectorAll('.tab_drawer_heading').forEach(h => {
                                        h.classList.remove('d_active');
                                    });
                                    document.querySelector(`.tab_drawer_heading[rel="${tabId}"]`).classList.add('d_active');
                                });
                            });
                            
                            document.querySelectorAll('.tab_drawer_heading').forEach(heading => {
                                heading.addEventListener('click', function() {
                                    ...
                                    document.querySelector(`.tabs-company li[rel="${tabId}"]`).classList.add('active');
                                });
                            });
                        }
                    })
