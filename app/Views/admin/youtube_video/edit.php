        ...
        <div class="card-body">
          ...
          <form action="<?= base_url('admin/youtube-videos/update/' . $video['id']) ?>" method="POST">
            <div class="row">
              <div class="col-md-6">
                <div class="form-group mb-3">
                  ...
                  <input type="text" class="form-control" id="video_id" name="video_id" value="<?= old('video_id', $video['fld_video_id']) ?>" required>
                  ...
                  <select class="form-select" id="product_id" name="product_id">
                    ...
                    <?php if (!empty($products)): ?>
                      <?php foreach ($products as $product): ?>
                        <option value="<?= $product['id'] ?>" <?= old('product_id', $video['fld_product_id']) == $product['id'] ? 'selected' : '' ?>>
                          <?= $product['fld_title'] ?>
                        </option>
                      <?php endforeach; ?>
                    <?php endif; ?>
            ...
            <div class="alert alert-info">
              <i class="bx bx-info-circle me-1"></i> Video details (title, description, views, posted date) will be automatically fetched from YouTube API when you update.
            </div>
